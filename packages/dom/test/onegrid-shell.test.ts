...
describe("@onegrid/dom renderer shell", () => {
  ...
  it("rejects segmented viewport rows with variable row height contracts", () => {
    const rowHeights = [() => 30, "auto" as const];
      ...
      expect(() => new OneGrid({
        ...
        dataSource: {
          async getRows(request) {
            return {
              rows: [{ id: `R-${request.startRow}` }],
              rowCount: 100_000_000
            };
          }
        }
      })).toThrow(/requires a fixed numeric rowHeight/);
    });
  });

  it("sizes merged row anchors from the row height resolver", () => {
    const el = document.createElement("div");
    const grid = new OneGrid({
      el,
      columns: [
        { field: "id", headerName: "ID" },
        { field: "stage", headerName: "Stage", merge: { mode: "value" } }
      ],
      data: [
        { id: "A", stage: "Capital", height: 30 },
        { id: "B", stage: "Capital", height: 60 },
        { id: "C", stage: "Capital", height: 45 }
      ],
      rowKey: "id",
      rowHeight: (row) => row.height
    });

    const mergedCell = el.querySelector<HTMLElement>('[data-column-id="stage"][aria-rowspan="3"]');

    expect(mergedCell?.style.getPropertyValue("--og-cell-row-span-height")).toBe("135px");
