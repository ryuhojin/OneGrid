...
test("export example imports CSV and XLSX files through public API", async ({ page }) => {
  ...
  await expect(page.getByRole("grid")).not.toContainText("EXP-0001");
  await expect(page.getByRole("grid")).toContainText("FILE-CSV-0002");
  const printDownload = await clickAndDownload(page, "Print layout", "onegrid-export.html");
  expectPrintHeaderLayout(await readDownload(printDownload));
  ...
  await expect(summaryValue(page, "Imported rows")).toHaveText("2");
  await expect(page.getByRole("grid")).not.toContainText("FILE-CSV-0001");
  await expect(page.getByRole("grid")).toContainText("FILE-XLS-0002");
});
...
test("XLSX file import replaces rows with raw file values", async ({ page }) => {
  ...
  await expect(page.getByRole("grid")).not.toContainText("EXP-0001");
  await expect(page.getByRole("grid")).toContainText("FILE-XLS-0002");
  await expect(page.getByRole("grid")).toContainText("1420");
  await expect(page.getByRole("grid")).toContainText("Approved");
});
